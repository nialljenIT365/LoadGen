...
The load generator exists to make Task Manager, Perfmon and nvidia-smi display chosen percentages so autoscale rules can be tested. A dial target therefore means the overall figure a Consumer shows, and the script contributes only the gap between the Baseline (OS, services, driver, compositor) and that figure, re-measuring every self-check tick. We rejected the deterministic alternative, where the script allocates or burns a flat percentage of the machine, because it would land the displayed figure above the target by an amount that drifts with background activity, which is exactly the number the tests care about.
